...
* **WICHTIGER HINWEIS: Dies ist eine Community Übersetzung und erhebt
...
Seit der Einführung von Ethereum im Jahr 2014 herrscht großes Interesse
an dezentralisierten, vertrauenslosen Anwendungen (sogenannten Smart
Contracts). Infolgedessen wurden viele Versuche unternommen, Anwendungen
mit realen Daten auf einer Blockchain zu implementieren. Wir glauben,
dass die Speicherung des Zustands und des Codes einer Anwendung auf der
Blockchain aus mehreren Gründen falsch ist. Dies bläht die Größe der
Blockchain auf, setzt Smart Contracts unnötigen Schwachstellen aus und
reduziert die Privatsphäre und verhindert so den Zugang von Blockchains
für private Anwendungsfälle (Banken, private Unternehmen, ...).

Aus diesem Grund wird æternity entwickelt, als die einzige Blockchain,
die durch ihr Kerndesign den Durchsatz von Mainstream Anwendungsfällen
aufrechterhalten kann, während sie Schnittstellen zur realen Welt
innerhalb ihrer Blockchain bereitstellt.

* Es kann dank der integrierten "State Channels" einen größeren
  Transaktionsdurchsatz verarbeiten (mehr als Visa und Legacy-Systeme).

* Die "State Channels" abseits der Blockchain ("off-chain") zu
  verarbeiten, erlaubt es, Smart Contracts einfacher zu analysieren,
  schneller zu verarbeiten und mehr Privatsphäre zu erstellen, während
  sie keinen wesentlichen Verlust an Funktionalität erleiden.

* Sein interner Konsensmechanismus kann das Orakel überprüfen und die
  Blockchain mit realen Daten versorgen, wobei darauf verzichtet wird
  ineffiziente Orakelmechanismen darauf aufbauend zu implementieren (wie
  Augur oder Gnosis auf Basis von Ethereum).

Anwendungen wie Märkte für synthetische Vermögenswerte und
Vorhersagemärkte können auf globaler Ebene effizient umgesetzt werden.
Für mehrere Teile gibt es bereits Implementierungen in Erlang, die die
Machbarkeit zeigen (Proof-of-Concept). Entwicklungswerkzeuge und
grundlegende Anwendungen wie ein Brieftaschen, Namens- und
Identitätssystem werden in Kürze zur Verfügung gestellt.
...
value exchange on the Internet [1](#quellen).
...
secured by a blockchain architecture [2](#quellen);
...
Casey Detrio showed how to make markets on blockchains [5](#quellen);
...
of existence for any digital data [7](#quellen).
...
contracts [8](#quellen), instead of utilizing the consensus mechanism
...
tokens.Sidenote: Originally æternity intended to be a 100
...
also known asweaksubjectivity[12](#quellen).
...
to represent things like persons or companies [13](#quellen).
...
bitcoins [14](#quellen), [15](#). These can be implemented using a
...
likelihood [3](#quellen), [8](#quellen), [16](#quellen).
...
[17](#quellen). Examples of Chalang code can also be found on our
Github<sup>4</sup>.
